...
import { Alert,Button, TextInput,Animated,Dimensions,View, Text, FlatList, Image, ActivityIndicator,StyleSheet,TouchableOpacity,Pressable , ScrollView} from 'react-native';
  ...
  const renderTextInputs = (viewIndex, inputs) => {
    return inputs.map(input => (
      <View key={input.id} style={{ flexDirection: 'row' }}>
        <TextInput style={{ flex: 1 , borderWidth:1, borderRadius:10,fontSize:18, fontWeight: "bold", marginLeft:20, padding: 10, marginTop: 10, }} />
        <TouchableOpacity onPress={() =>  handleDeleteInput(viewIndex, input.id)}>
            <AntDesign name="delete" size={25} color="black" marginLeft={10} marginTop={20} mar  />
            </TouchableOpacity>
      </View>
    ));
  };
  ...
  return (
    <View>
       ...
       <View style={{alignItems: 'center',justifyContent:'center'}} >
        <TouchableOpacity style={{height:240, width:"100%", backgroundColor:"gray", alignItems: 'center',justifyContent:'center'}} onPress={handlePress} >
       ...
       <View style={{}}>
        <Text style={{ fontSize: 20, fontWeight:"bold", marginLeft:20}}>Nguyên Liệu</Text>
        {renderTextInputs(1, textInputs1)}
       </View>  
       <View>
       <TouchableOpacity style={{ marginLeft : "30%",borderWidth:1, borderRadius:10, flexDirection:"row", width:"45%",    justifyContent: 'space-between',alignItems: 'center',marginTop:10}} onPress={handleAddInput1} >
       <MaterialIcons name="add" size={30}/>
           <Text style={{ fontSize:18, fontWeight:"bold", padding:10}}>Nguyên Liệu</Text>
        </TouchableOpacity>
     </View>
      <View>
      <View style={{backgroundColor: "gray", height: 10, marginTop:10}}></View>
      <Text style={{ fontSize: 20, fontWeight:"bold", marginLeft:20}}>Cách làm</Text>
      {renderTextInputs(2, textInputs2)}
        <TouchableOpacity style={{ marginLeft : "30%",borderWidth:1, borderRadius:10, flexDirection:"row", width:"45%",    justifyContent: 'space-between',alignItems: 'center',marginTop:10}} title="Show Text" onPress={handleAddInput2} >
        <MaterialIcons name="add" size={30}/>
